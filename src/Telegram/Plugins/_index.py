...
from ._file import handle_file_task
...
@command(command='dropmovies', description="Drop Movies from database (Sudo only)",Custom_filter=CustomFilters.authorize(sudo=True))
@round_robin()
async def drop_database(client: Client, message: Message) -> None:
    database.Movies.dropAll()
